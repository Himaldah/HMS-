...
// Fetch Appointments
// $appointments = $conn->query("SELECT appointments.*, doctors.drname, doctors.did FROM appointments JOIN doctors ON appointments.drid = doctors.drid WHERE appointments.pid = '$patient_id' ORDER BY aid DESC, appointment_time DESC");

// Initialize base query
$filter_sql = "SELECT appointments.*, doctors.drname FROM appointments JOIN doctors ON appointments.drid = doctors.drid WHERE appointments.pid = '$patient_id'";
...
$filter_sql .= " ORDER BY aid DESC, appointment_time DESC";
...
if (isset($_GET['action']) && isset($_GET['aid'])) {
    ...
    if ($action === 'continue') {
        mysqli_query($conn, "UPDATE appointments SET status = 'Pending' WHERE aid = $aid");
        $_SESSION['toast-2'] = ['message' => 'Appointment has been continued.', 'type' => 'info'];
    } elseif ($action === 'cancel') {
        mysqli_query($conn, "UPDATE appointments SET status = 'Cancelled' WHERE aid = $aid");
        $_SESSION['toast-2'] = ['message' => 'Appointment has been cancelled.', 'type' => 'error'];
    } 
...
?>
    ...
    <header class="text-black text-center py-2 pt-20">
        <h1 class="text-3xl text-center font-bold text-blue-900 ">Appointments List</h1>
    </header>
